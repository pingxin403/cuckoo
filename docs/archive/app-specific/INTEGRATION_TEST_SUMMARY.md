...
Uses Docker Compose files in `deploy/docker/` for service orchestration.
...
2. Starts MySQL and Redis from `deploy/docker/docker-compose.infra.yml`
...
docker compose -f deploy/docker/docker-compose.services.yml up -d shortener-service
...
- Docker Compose files in `deploy/docker/` - Added MySQL, Redis, and shortener-service
