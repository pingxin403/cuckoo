...
func main() {
	...
	// Create service
	svc := service.NewUimUserviceServiceServer(store)

	// Create gRPC server
	grpcServer := grpc.NewServer()

	// Register service
	im_servicepb.RegisterUimUserviceServiceServer(grpcServer, svc)
	...
	go func() {
		log.Printf("im-service listening on port %s", port)
	...
	log.Println("im-service shutdown complete")
}
