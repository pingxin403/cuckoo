...
- `im-service` → Your service name (e.g., `user-service`)
- `IM message routing and delivery service` → Brief description of your service
- `9094` → gRPC port number (e.g., `9092`)
- `github.com/pingxin403/cuckoo/apps/im-service` → Go module path (e.g., `github.com/myorg/myrepo/apps/user-service`)
- `im_service` → Protobuf file name (e.g., `user.proto`)
- `im_servicepb` → Protobuf package name (e.g., `userpb`)
...
#### go.mod
```go
module github.com/pingxin403/cuckoo/apps/im-service
...
Create your service's Protobuf definition in `api/v1/im_service`:
...
option go_package = "github.com/myorg/myrepo/apps/im-service/gen/im_servicepb";

service UimUserviceService {
...
# Or manually
protoc --go_out=apps/im-service/gen \
       --go_opt=paths=source_relative \
       --go-grpc_out=apps/im-service/gen \
...
Update `service/im-service_service.go`:
...
type YourStore interface {
    Create(item *im_servicepb.YourItem) error
    Get(id string) (*im_servicepb.YourItem, error)
    List() ([]*im_servicepb.YourItem, error)
    Update(item *im_servicepb.YourItem) error
    Delete(id string) error
}

type MemoryStore struct {
    mu    sync.RWMutex
    items map[string]*im_servicepb.YourItem
}

func NewMemoryStore() *MemoryStore {
    return &MemoryStore{
        items: make(map[string]*im_servicepb.YourItem),
    }
}
...
metadata:
  name: im-service
  description: IM message routing and delivery service
  tags:
    - go
    - grpc
spec:
...
```makefile
build-im-service:
	@echo "Building im-service..."
	cd apps/im-service && go build -o bin/im-service .

test-im-service:
	@echo "Testing im-service..."
	cd apps/im-service && go test ./...
```
...
- `PORT`: gRPC server port (default: `9094`)
...
import (
    ...
    yourpb "github.com/pingxin403/cuckoo/apps/im-service/gen/im_servicepb"
    "google.golang.org/grpc"
    "google.golang.org/grpc/credentials/insecure"
)

var grpcAddr = getEnv("GRPC_ADDR", "localhost:9094")
...
docker compose up -d im-service
...
# Run tests
cd "apps/im-service"
GRPC_ADDR="localhost:9094" go test -v ./integration_test/... -count=1 -timeout 5m

# Cleanup
cd "$PROJECT_DIR"
docker compose stop im-service
```
...
```bash
docker run -p 9094:9094 im-service:latest
```
